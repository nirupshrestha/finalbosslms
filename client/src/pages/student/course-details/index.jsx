...
function StudentViewCourseDetailsPage() {
  ...
  async function fetchStudentViewCourseDetails() {
    ...
    if (auth?.authenticate && auth?.user?._id) {
      ...
      if (
        ...
        // User has purchased this course, navigate to progress page
        navigate(`/student/course-progress/${currentCourseDetailsId}`);
        return;
      }
    }

    // Fetch course details regardless of login status
  ...
  function handleBuyNowClick() {
    ...
    if (!auth?.authenticate || !auth?.user) {
      // Store the course ID in sessionStorage for redirection after login
      sessionStorage.setItem("redirectAfterLogin", `/course/details/${currentCourseDetailsId}`);
          ...
          <Card className="sticky top-4">
            <CardContent className="p-6">
              ...
              <Button onClick={handleBuyNowClick} className="w-full">
                {auth?.authenticate ? "Buy Now" : "Sign In to Purchase"}
              </Button>
            </CardContent>
          </Card>
        </aside>
